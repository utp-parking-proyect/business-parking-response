...
import com.utp.response.generated.model.ApiExceptionDetail;
...
public class ErrorResponseHandler {
  ...
  public Mono<ResponseEntity<ModelApiException>> handleValidationError(IllegalArgumentException e) {
    ...
    return Mono.just(ResponseEntity.badRequest().body(buildFunctionalError(e.getMessage())));
  }
  ...
  public Mono<ResponseEntity<ModelApiException>> handleTimeout(TimeoutException e) {
    log.error("business-core-portal no respondió a tiempo: {}", e.getMessage());
    return Mono.just(ResponseEntity.status(HttpStatus.GATEWAY_TIMEOUT)
        .body(buildFunctionalError(Constants.ERROR_USERS_SERVICE_TIMEOUT)));
  }
